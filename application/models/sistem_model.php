...
class Sistem_model extends CI_Model {
	...
	function CekLogin($data) {
		...
		$ceklogin = $this->db->query("select a.*,b.* from tbl_user a join tbl_user_group b on 
			a.user_group_id=b.id_user_group where a.username_user='$username_user' and a.password_user= 
			'$password_user' ");
...
//Akhir kategori galeri
	...
	function EditKelasKamar($id){
		return $this->db->query("select * form tbl_kelas_kamar where id_kelas_kamar='$id' ");
	}
	...
	//Awal kamar
	function Kamar() {
		return $this->db->query("select a.*,b.* from tbl_kamar a join tbl_kelas_kamar b on 
			a.kelas_kamar_id=b.id_kelas_kamar order by a.id_kamar desc");
	}
	...
	function KamarId($id) {
		return $this->db->query("select a.*,b.* from tbl_kamar a join tbl_kelas_kamar b on 
			a.kelas_kamar_id=b.id_kelas_kamar where a.id_kamar='$id' ");
	}
...
//Awal user 
	function User(){
	return $this->db->query("select a.*,b.* from tbl_user a join tbl_user_group b on a.user_group_id=b.id_user_group order by a.id_user desc");
	}
	...
	//Awal reservasi 
	function Reservasi(){
		return $this->db->query("select a.*,b.* from tbl_reservasi a join tbl_kamar b on a.kamar_id=b.id_kamar order by a.id_reservasi desc");
	}
	//Akhir reservasi 

	//Awal New Reservasi 
	function NewReservasi() {
		return $this->db->query("select a.*,b.* from tbl_reservasi a join tbl_kamar b on a.kamar_id=b.id_kamar where a.status_reservasi='0' or a.status_reservasi='1' or a.status_reservasi='2' order by a.id_reservasi desc"); 
	}

	function Kamarkosong() {
		return $this->db->query("select a.*,b.* from tbl_kamar a join tbl_kelas_kamar b on a.kelas_kamar_id=b.id_kelas_kamar where status_kamar='0' order by a.id_kamar,a.kelas_kamar_id desc");
	}

	function ReservasiId($id){
		return $this->db->query("select a.*,b.*,TIMESTAMPDIFF(DAY, a.tgl_reservasi_masuk, a.tgl_reservasi_keluar) as waktu from tbl_reservasi a join tbl_kamar b on a.kamar_id=b.id_kamar where id_reservasi='$id' ");
	}

	function NewReservasiBaru() {
		return $this->db->query("select a.*,b.* from tbl_reservasi a join tbl_kamar b on a.kamar_id=b.id_kamar where a.status_reservasi='0' order by a.id_reservasi desc"); 
	}
	//Akhir New Reservasi 
